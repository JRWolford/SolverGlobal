...
namespace Api.Database
{
    ...
    public class SqlServerDataRepository : IDataRepository
    {
        ...
        //ToDo: fix schema bug, DBO vs SalesLT can use same table name dbo.Table SalesLT.Table
        ...
        public async Task<CompactDataResponse> GetCompactDataAsync(string tableName)
        {
            var tableSchema = await GetTableSchemaAsync(tableName);
            var sqlQuery = $"SELECT * FROM {tableSchema}.{tableName} WHERE 1=1";

            async Task<CompactDataResponse> Command(SqlConnection sqlConnection)
            {
                var sqlCommand = new SqlCommand(sqlQuery, sqlConnection);
                var reader = await sqlCommand.ExecuteReaderAsync();
                ...
                var data = new CompactDataResponse();
                ...
                while (await reader.ReadAsync())
                {
                    ...
                    if(data.Fields.Count == 0)
                        ...
                        {
                            data.Fields.Add(reader.GetName(i));
                        }
                    var record = new List<object>();
                    for (var i = 0; i < data.Data.Count; i++)
                    {
                        record.Add(reader[data.Fields[i]]);
                    }
        ...
        public async Task<CompactDataResponse> GetPagedCompactDataAsync(string tableName, uint pageSize, uint pageNumber)
        {
            var tableSchema = await GetTableSchemaAsync(tableName);
            var sqlQuery = $"SELECT * FROM {tableSchema}.{tableName} ORDER BY 1 OFFSET @pageNumber ROWS FETCH NEXT @pageSize ROWS ONLY";


            async Task<CompactDataResponse> Command(SqlConnection sqlConnection)
            {
                var sqlCommand = new SqlCommand(sqlQuery, sqlConnection);
                sqlCommand.Parameters.AddWithValue("@pageSize", (int) (pageSize));
                sqlCommand.Parameters.AddWithValue("@pageNumber", (int) pageSize * (pageNumber - 1));
